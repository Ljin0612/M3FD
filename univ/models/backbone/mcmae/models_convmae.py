...
#        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        ...
        for i,blk in enumerate(self.blocks3):
            if i < len(self.blocks3) - 1:
                ...
                if not return_attention_map:
                    ...
                    x , last_attention_map =  blk(x, return_attention=True)
        # x = x + stage1_embed + stage2_embed
        # NOTE : use this clip loss   main_daig is NAN
        x = self.norm(x)

        return x, mask, ids_restore ,last_attention_map
    ...
    def forward(self, imgs,mask_ratio=0.75 , return_last_attention = False):
        """
            args:
                imgs: [N, 3, H, W]
                return_last_attention : the demand of last attention map
            return:
                latent : last layer output : patch embed
                attention_map : if return_last_attention : attention map  else  None
        """
        latent, _ , _ , attention_map = self.forward_encoder(imgs, mask_ratio , return_attention_map=return_last_attention)
        return latent, attention_map
